...
describe('Connectivity Exceptions', () => {
  describe('ConnectivityException', () => {
    ...
    it('should create exception with host and error message', () => {
      const exception = new ConnectivityException(
        'example.com',
        'Connection refused',
      );
      ...
      expect(exception.message).toBe(
        'Host example.com is not accessible: Connection refused',
      );
    });
  ...
  describe('TimeoutException', () => {
    it('should create timeout exception with host and timeout duration', () => {
      ...
      expect(exception.message).toBe(
        'Connection to 192.168.1.100 timed out after 5000ms',
      );
    });
    ...
    it('should handle zero timeout', () => {
      ...
      expect(exception.message).toBe(
        'Connection to api.example.com timed out after 0ms',
      );
    });
    ...
    it('should handle very large timeout values', () => {
      ...
      expect(exception.message).toBe(
        'Connection to slow-server.com timed out after 300000ms',
      );
    });
  });
  ...
  describe('HostUnreachableException', () => {
    it('should create host unreachable exception with IP address', () => {
      ...
      expect(exception.message).toBe(
        'Host 10.20.30.40 is unreachable - please check network connectivity',
      );
    });
    ...
    it('should create host unreachable exception with domain name', () => {
      ...
      expect(exception.message).toBe(
        'Host unreachable.example.com is unreachable - please check network connectivity',
      );
    });
    ...
    it('should handle localhost', () => {
      ...
      expect(exception.message).toBe(
        'Host localhost is unreachable - please check network connectivity',
      );
    });
  });
});
