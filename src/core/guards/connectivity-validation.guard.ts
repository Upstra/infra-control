...
export class ConnectivityValidationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!host) {
      ...
      throw new BadRequestException(`Host not provided in ${requirement.hostSource}.${requirement.hostField}`);
    }
    ...
    try {
      ...
      this.logger.log(`Connectivity validated for ${host} (${result.responseTime}ms)`);
      return true;
    } catch (error) {
      this.logger.error(`Connectivity validation failed for ${host}: ${error.message}`);
      throw new BadRequestException(`Cannot reach host ${host}: ${error.message}`);
    }
  }

  private extractHost(request: any, requirement: ConnectivityRequirement): string | null {
