...
import { createMockPermissionServer, createMockPermissionVm } from '@/modules/permissions/__mocks__/permissions.mock';
...
describe('PermissionAggregateService', () => {
  it('aggregates server permissions by serverId', () => {
    const perms = [
      createMockPermissionServer({ serverId: 'srv1', bitmask: PermissionBit.READ }),
      createMockPermissionServer({ serverId: 'srv1', bitmask: PermissionBit.WRITE }),
      createMockPermissionServer({ serverId: 'srv2', bitmask: PermissionBit.DELETE }),
      createMockPermissionServer({ serverId: null, bitmask: PermissionBit.RESTART }),
      createMockPermissionServer({ serverId: null, bitmask: PermissionBit.SHUTDOWN }),
    ];
    ...
    expect(global?.bitmask).toBe(PermissionBit.RESTART | PermissionBit.SHUTDOWN);
  ...
  it('aggregates vm permissions by vmId', () => {
    const perms = [
      ...
      createMockPermissionVm({ vmId: undefined, bitmask: PermissionBit.RESTART }),
    ];
