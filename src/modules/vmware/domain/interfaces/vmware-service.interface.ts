...
export interface IVmwareService {
  ...
  getVMMetrics(moid: string, connection: VmwareConnectionDto): Promise<VmwareVmMetrics>;
  controlVMPower(
    moid: string,
    action: 'on' | 'off',
  ...
  getHostMetrics(moid: string, connection: VmwareConnectionDto): Promise<VmwareHost>;
}