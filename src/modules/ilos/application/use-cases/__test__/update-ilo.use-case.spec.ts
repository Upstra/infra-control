...
describe('UpdateIloUseCase', () => {
  ...
  beforeEach(() => {
    repo = { findIloById: jest.fn(), findIloByIdWithCredentials: jest.fn(), save: jest.fn(), updateIlo: jest.fn() };
  ...
  it('should update and return IloResponseDto', async () => {
    ...
    repo.updateIlo.mockResolvedValue({ ...entity, name: 'newilo', ip: '10.0.0.9' });
  ...
  it('should throw if not found', async () => {
    ...
    await expect(useCase.execute({ id: 'bad-id' })).rejects.toThrow('Not found');
  });
  ...
  it('should throw if repository.updateIlo fails', async () => {
    ...
    await expect(useCase.execute({ id: 'ilo-id', name: 'err' })).rejects.toThrow(
      'Update failed',
    );
  });
});
